...
test.describe('Feature: Invite a Friend (Invitaciones)', () => {
    ...
    test.beforeEach(async ({ page, context }) => {
        ...
        await context.route('**/api/settings', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ site_name: 'Sushi de Maksim', min_order: 20 }) }));
        await context.route('**/api/user/active', route => route.fulfill({ status: 200, contentType: 'application/json', body: '{"user":null}' }));
        await context.route('**/api/user/favorites', route => route.fulfill({ status: 200, contentType: 'application/json', body: '{"favorites":[]}' }));
        
        await context.route('**/api/menu*', route => route.fulfill({ 
            status: 200, 
        ...
        await context.route('**/api/cart', route => {
            return route.fulfill({ 
                ...
                contentType: 'application/json',
                body: JSON.stringify({ 
                    items: [{ menu_item_id: 1, name: 'Gyozas con carne', price: 6.90, quantity: 3, category: 'entrantes', image: '', description: '' }],
                    total: 20.70
                }) 
            });
        });
    ...
    test('SUCCESS: Create invitation and pay as friend', async ({ page, context }) => {
        ...
        await context.route('**/api/auth/me', async (route) => {
            ...
            if (auth?.includes('sender-token')) {
                await route.fulfill({
                    ...
                    contentType: 'application/json',
                    body: JSON.stringify({ 
                        user: { id: 101, name: 'Sender', full_name: 'Sender', email: 'sender@test.com', role: 'user', addresses: [] } 
                    })
                });
            } else {
        ...
        await context.route('**/api/auth/login', route => route.fulfill({
            ...
            body: JSON.stringify({ 
                token: 'sender-token', 
                user: { id: 101, name: 'Sender', full_name: 'Sender', email: 'sender@test.com' } 
            })
        }));
        ...
        await context.route('**/api/orders/invite', route => route.fulfill({ 
            status: 200, 
            contentType: 'application/json',
        ...
        // 3. Recipient part
        await context.route(`**/api/orders/public/${fakeOrderId}`, route => route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({
                order: {
                    ...
                    items: [{ name: 'Gyozas con carne', quantity: 3, price: 6.90, image: '' }]
                }
            })
        }));
        ...
        await expect(recipientPage.getByText(/Momento de invitar/i).first()).toBeVisible({ timeout: 30000 });

        await context.route(`**/api/orders/*/confirm-payment`, route => route.fulfill({ 
            status: 200, 
            contentType: 'application/json',
            body: '{"success":true}' 
        }));
        ...
        await expect(recipientPage.locator('h2', { hasText: /¡Eres Genial!/i })).toBeVisible({ timeout: 20000 });
    });
    ...
    test('PROTECTION: Guests cannot invite friends (prompt to login)', async ({ page }) => {
        ...
        await page.evaluate(() => {
            window.localStorage.setItem('guest_cart', JSON.stringify([
                { id: '1', name: 'Gyozas', price: 10, quantity: 1, category: 'entrantes', image: '', description: '' }
            ]));
        });
