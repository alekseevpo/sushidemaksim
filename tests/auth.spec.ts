...
test.describe('Authentication Flow', () => {
    ...
    test('SUCCESS: should register and see the welcome message', async ({ page }) => {
        await page.route('**/api/auth/me', route => route.fulfill({ status: 401, body: '{"error":"No"}' }));
        await page.route('**/api/auth/register', route => route.fulfill({ status: 200, body: '{"success":true}' }));
        ...
        await expect(page.getByText(/enviado|revisa|email/i).first()).toBeVisible({ timeout: 15000 });
    });
        ...
        await page.route('**/api/auth/me', async (route) => {
            ...
            if (h['authorization'] || h['Authorization']) {
                await route.fulfill({
                    ...
                    contentType: 'application/json',
                    body: JSON.stringify({ 
                        user: { id: 1, name: 'Pavel', full_name: 'Pavel', email: 'test@test.com', role: 'user', addresses: [] } 
                    })
                });
            } else {
    ...
    test('FAILURE: should show error on invalid credentials', async ({ page }) => {
        ...
        await page.getByRole('button', { name: /Iniciar sesión/i }).click();
        await expect(page.getByText(/incorrect|inválid|incorrectas/i).first()).toBeVisible({ timeout: 10000 });
    });
});
