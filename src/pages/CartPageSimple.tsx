...
export default function CartPageSimple() {
  const { items, total, updateQuantity, removeItem, clearCart, isLoading: cartLoading } = useCart();
  ...
  const handleOrder = async () => {
    setOrderError('');
    const deliveryAddress = address.trim() || (defaultAddr ? `${defaultAddr.street}, ${defaultAddr.postalCode} ${defaultAddr.city}` : '');
    const deliveryPhone = phone.trim() || user?.phone || '';

    if (!deliveryAddress) { setOrderError('Por favor, introduce una dirección de entrega'); return; }
    if (!deliveryPhone) { setOrderError('Por favor, introduce un teléfono de contacto'); return; }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-2">
            Pedido <span className="font-bold text-gray-900">#{String(orderSuccess).padStart(5, '0')}</span>
          </p>
          <p className="text-gray-500 mb-8">
            Tiempo estimado de entrega: <span className="font-bold text-red-600">30–60 min</span>
          </p>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <div className="bg-white rounded-xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1)] p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold m-0">Productos ({items.length})</h2>
                <button
                  onClick={clearCart}
                  ...
                  <div key={item.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-lg font-bold text-red-600 m-0">
                        {item.price.toFixed(2).replace('.', ',')} € × {item.quantity} = {(item.price * item.quantity).toFixed(2).replace('.', ',')} €
                      </p>
                    </div>
            ...
            {isAuthenticated && (
              <div className="bg-white rounded-xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1)] p-6">
                ...
                {defaultAddr && (
                  <button
                    onClick={() => {
                      setAddress(`${defaultAddr.street}, ${defaultAddr.postalCode} ${defaultAddr.city}`);
                  ...
                  >
                    <MapPin size={14} /> Usar "{defaultAddr.label}": {defaultAddr.street}
                  </button>
                )}
                ...
                <div className="flex flex-col gap-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder={defaultAddr ? `${defaultAddr.street}, ${defaultAddr.postalCode} ${defaultAddr.city}` : 'Calle, número, piso, ciudad'}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-600 mb-1">Teléfono de contacto *</label>
                    <input
                      type="tel"
                      value={phone}
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <label className="block text-sm font-semibold text-gray-600 mb-1">Comentario para el pedido (Opcional)</label>
                  <textarea
                    value={customNote}
          ...
          <div>
            <div className="bg-white rounded-xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1)] p-6 sticky top-24">
                ...
                <div className="border-t border-gray-200 pt-3 mt-1">
                  <div className="flex justify-between text-lg font-bold">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-red-600">{finalTotal.toFixed(2).replace('.', ',')} €</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6 pb-6 border-b border-gray-100">
                <label className="block text-sm font-semibold text-gray-600 mb-2">Código promocional</label>
                {!appliedPromo ? (
                  ...
                  <div className="flex items-center justify-between bg-green-50 border border-green-200 px-4 py-3 rounded-lg">
                    <div className="text-sm text-green-700 font-medium">
                      🎉 -{appliedPromo.percentage}% (Código: {appliedPromo.code})
                    </div>
                    <button onClick={() => setAppliedPromo(null)} className="text-red-600 text-sm font-bold hover:underline">
                      Quitar
                    </button>
                  </div>
                )}
