...
export default function AddressesTab({ addresses, addAddress, editAddress, removeAddress, setDefaultAddress, onSuccess }: Props) {
    const [showAddAddress, setShowAddAddress] = useState(false);
    const [editId, setEditId] = useState<string | null>(null);
    const [newAddress, setNewAddress] = useState({
        label: '', street: '', city: 'Madrid', postalCode: '', phone: '', isDefault: false,
    });
    ...
    const resetForm = () => {
        setNewAddress({ label: '', street: '', city: 'Madrid', postalCode: '', phone: '', isDefault: false });
        setSearchQuery('');
        setHouseNumber('');
        setDetails('');
        setEditId(null);
    ...
    const handleSaveAddress = async () => {
        if (!newAddress.label || !newAddress.street || !newAddress.postalCode || !newAddress.phone) return;
        try {
            const finalStreet = [newAddress.street, houseNumber.trim(), details.trim()].filter(Boolean).join(', ');
    ...
    return (
        <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0, color: '#111827' }}>Mis Direcciones</h1>
                <button
                    onClick={() => setShowAddAddress(true)}
                    style={{
                        display: 'flex', alignItems: 'center', gap: '6px',
                        padding: '8px 16px', border: 'none', borderRadius: '8px',
                        cursor: 'pointer', backgroundColor: '#DC2626', fontSize: '14px', color: 'white',
                        fontWeight: 'bold',
                    }}
                >
            ...
            {showAddAddress && (
                <div style={{ ...cardStyle, border: '2px solid #DC2626' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                        <div>
                            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#6B7280', marginBottom: '4px' }}>
                                Nombre (ej: Casa, Oficina)
                            </label>
                            <input value={newAddress.label} onChange={e => setNewAddress(p => ({ ...p, label: e.target.value }))} placeholder="Casa" style={inputStyle} />
                        </div>
                        <div>
                            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#6B7280', marginBottom: '4px' }}>
                                Teléfono de contacto
                            </label>
                            <input value={newAddress.phone} onChange={e => setNewAddress(p => ({ ...p, phone: e.target.value }))} placeholder="+34 600 000 000" style={inputStyle} />
                        </div>
                    </div>
                                    ...
                                    {suggestions.map((s, i) => (
                                        <button
                                            ...
                                            style={{
                                                ...
                                                textAlign: 'left', fontSize: '13px', color: '#374151',
                                                borderBottom: i < suggestions.length - 1 ? '1px solid #F3F4F6' : 'none',
                                            }}
                                            onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#F9FAFB')}
                                            onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
                                        >
                                            <MapPin size={14} style={{ color: '#DC2626', marginTop: '2px', flexShrink: 0 }} />
                    ...
                    <div style={{ marginBottom: '12px' }}>
                        <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#6B7280', marginBottom: '4px' }}>
                            Piso, Escalera, Puerta <span style={{ color: '#9CA3AF', fontWeight: '400' }}>(opcional)</span>
                        </label>
                        <input
                            value={details}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                        <div>
                            ...
                            <input value={newAddress.city} onChange={e => setNewAddress(p => ({ ...p, city: e.target.value }))} style={inputStyle} />
                        </div>
                        <div>
                            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#6B7280', marginBottom: '4px' }}>
                                Código postal
                            </label>
                            <input value={newAddress.postalCode} onChange={e => setNewAddress(p => ({ ...p, postalCode: e.target.value }))} placeholder="28001" style={inputStyle} />
                        </div>
                    </div>

                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', cursor: 'pointer', fontSize: '14px' }}>
                        <input type="checkbox" checked={newAddress.isDefault} onChange={e => setNewAddress(p => ({ ...p, isDefault: e.target.checked }))} style={{ accentColor: '#DC2626' }} />
            ...
            {addresses.length === 0 && !showAddAddress ? (
                ...
                addresses.map(addr => (
                    <div key={addr.id} style={{ ...cardStyle, display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                        <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                <h3 style={{ fontSize: '16px', fontWeight: 'bold', margin: 0, color: '#111827' }}>{addr.label}</h3>
                                {addr.isDefault && (
                                    <span style={{
                                        backgroundColor: '#DCFCE7', color: '#16A34A', fontSize: '11px',
                                        fontWeight: 'bold', padding: '2px 8px', borderRadius: '9999px',
                                    }}>
                                        Predeterminada
                                    </span>
                                )}
                            </div>
                            <p style={{ fontSize: '14px', color: '#374151', margin: '0 0 2px 0' }}>{addr.street}</p>
                            <p style={{ fontSize: '13px', color: '#6B7280', margin: '0 0 2px 0' }}>{addr.postalCode} {addr.city}</p>
                            <p style={{ fontSize: '13px', color: '#6B7280', margin: 0 }}>{addr.phone}</p>
                        </div>
