...
export default function FavoritesTab() {
    ...
    if (loading) {
        return (
            <div>
                <h1 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '20px', color: '#111827' }}>Mis Favoritos</h1>
                <div style={{ ...cardStyle, textAlign: 'center', padding: '48px 24px' }}>
                    <div style={{ fontSize: '32px', marginBottom: '12px' }} className="animate-pulse">⏳</div>
            ...
            <div>
                ...
                <div style={{ ...cardStyle, textAlign: 'center', padding: '48px 24px' }}>
                    ...
                    <p style={{ fontSize: '14px', color: '#6B7280' }}>
                        Puedes añadir tus platos preferidos desde el Menú usando el botón del corazón.
                    </p>
                </div>
            </div>
        );
    ...
    return (
        <div>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '20px', color: '#111827' }}>Mis Favoritos ({favorites.length})</h1>
                    ...
                    >
                        ...
                        <div className="p-4 flex flex-col flex-1">
                            <div className="flex items-start justify-between mb-2">
                                <h3 className="text-[17px] font-bold m-0 text-gray-900 leading-tight">{item.name}</h3>
                                <div className="flex gap-1 ml-2 flex-shrink-0">
                                    {item.vegetarian && <span className="text-emerald-500 text-sm">🥬</span>}
                                </div>
                            </div>
                            <p className="text-gray-500 text-[13px] leading-relaxed mb-3 flex-1 m-0">{item.description}</p>
                            {item.pieces && (
                                <div className="text-[13px] text-gray-400 mb-3 font-medium">{item.pieces} uds</div>
                            )}
                            <div className="flex items-center justify-between mt-auto">
                                ...
                                <button
                                    ...
                                    className={`px-3 py-2 rounded-lg font-bold border-none cursor-pointer flex items-center gap-1.5 text-[13px] outline-none transition-all duration-300 ${addedItems.has(item.id)
                                        ? 'bg-green-600 text-white'
                                        : 'bg-red-600 text-white hover:bg-red-700 shadow-sm'
                                        }`}
                                >
                                    {addedItems.has(item.id) ? <>✓ Añadido</> : <><Plus size={14} />Añadir</>}
                                </button>
                            </div>
                        </div>
                    </div>
