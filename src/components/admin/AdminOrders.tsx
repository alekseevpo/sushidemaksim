...
export default function AdminOrders() {
    ...
    const statusOptions = [
        ...
        { value: 'confirmed', label: 'Confirmado', color: 'bg-indigo-100 text-indigo-700 border-indigo-200' },
        { value: 'preparing', label: 'Preparando', color: 'bg-blue-100 text-blue-700 border-blue-200' },
        { value: 'on_the_way', label: 'En camino', color: 'bg-purple-100 text-purple-700 border-purple-200' },
                ...
                <div className="space-y-4">
                    {filteredOrders.length === 0 ? (
                        ...
                        filteredOrders.map(order => (
                            <div key={order.id} className="bg-white border text-left border-gray-200 shadow-sm rounded-xl overflow-hidden hover:shadow-md transition">
                                <div className="p-4 sm:p-5 bg-gray-50 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                    <div>
                                        ...
                                        <div className="text-sm font-medium text-gray-600">
                                            <strong>Total:</strong> <span className="text-red-600 font-bold">{formatCurrency(order.total)}</span>
                                            {order.promocode && (
                                                <span className="ml-2 bg-green-100 text-green-800 px-2 py-0.5 rounded text-xs">Promo: {order.promocode}</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
                                        ...
                                        <select
                                            ...
                                            className={`text-sm font-bold border-2 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-offset-1 transition appearance-none cursor-pointer ${statusOptions.find(opt => opt.value === order.status)?.color || 'bg-gray-100'
                                                }`}
                                        >
                                ...
                                <div className="p-4 sm:p-5 grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                        <div>
                                            <h4 className="text-xs uppercase font-bold text-gray-400 mb-2">Cliente y Entrega</h4>
                                            <div className="bg-gray-50 rounded-lg p-3 text-sm flex flex-col gap-1 border border-gray-100">
                                                <span><strong>Nombre:</strong> {order.user_name}</span>
                                                <span><strong>Email:</strong> <span className="text-gray-500">{order.user_email}</span></span>
                                                <span><strong>Dirección:</strong> {order.delivery_address}</span>
                                                <span><strong>Teléfono:</strong> <a href={`tel:${order.phone_number}`} className="text-blue-600 hover:underline">{order.phone_number}</a></span>
                                            </div>
                                        </div>
                                        {order.notes && (
                                            <div>
                                                <h4 className="text-xs uppercase font-bold text-gray-400 mb-2">Comentarios</h4>
                                    ...
                                    <div>
                                        <h4 className="text-xs uppercase font-bold text-gray-400 mb-2">Artículos ({order.items?.length || 0})</h4>
                                            ...
                                            {order.items?.map((item: any) => (
                                                <div key={item.id} className="p-3 flex items-center justify-between text-sm">
                                                    <div className="flex items-center gap-3">
                                                            ...
                                                            {item.image ? (
                                                                <img
                                                                    ...
                                                                    onError={(e) => {
                                                                        ...
                                                                        if (p) p.innerHTML = '<span class="text-xs">🍣</span>';
                                                                    }}
                                                                />
                                                            ) : (
                                                        ...
                                                        <span className="font-bold text-gray-800">{item.name} <span className="text-gray-400 font-normal">x{item.quantity}</span></span>
                                                    </div>
                                                    <span className="font-bold text-gray-600">{formatCurrency(item.price_at_time * item.quantity)}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
