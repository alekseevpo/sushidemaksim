    ...
    return (
        <aside className="hidden lg:block w-[220px] flex-shrink-0 bg-red-600 min-h-full z-30">
            <div className="sticky top-[64px] h-[calc(100vh-64px)] pb-8 px-3 flex flex-col items-stretch overflow-y-auto no-scrollbar rounded-none">
                ...
                <div className="mt-8 px-4 flex flex-col items-stretch gap-4 pb-2">
                    <button
                        onClick={() => {
                            // Since we don't have setIsReservationModalOpen here directly,
                            // we can dispatch a custom event or the parent could pass it.
                            // But MenuCategoryBar is used in MenuPageSimple which doesn't have it either.
                            // Wait, Header.tsx uses ReservationModal.
                            // I'll just find if there's a better way.
                            // For now, let's use a custom event.
                            window.dispatchEvent(new CustomEvent('open:reservation'));
                        }}
                        className="w-full bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-2xl py-4 px-2 font-black text-[12px] tracking-widest transition-all cursor-pointer flex items-center justify-center gap-2"
                    >
                        <svg
                            viewBox="0 0 24 24"
                            width="16"
                    ...
                    <div className="flex items-center justify-center pointer-events-none select-none pt-4 opacity-50">
                        <span className="text-white text-6xl font-serif drop-shadow-[2px_2px_3px_rgba(0,0,0,0.4)]">
                            福
                        </span>
                    </div>
                </div>
