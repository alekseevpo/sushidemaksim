...
import { Mail, Lock, User, Phone, Eye, EyeOff, CheckCircle, ArrowLeft, KeyRound } from 'lucide-react';
...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const resetForm = () => {
    ...
    setName(''); setPhone(''); setError(''); setSuccess('');
    setShowPassword(false); setShowNewPassword(false); setRecoveryEmail('');
    setResetCode(''); setMode('login');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      if (password.length < 6) { setError('La contraseña debe tener al menos 6 caracteres'); setIsLoading(false); return; }
  ...
  const handleResetSubmit = async (e: React.FormEvent) => {
    ...
    setError(''); setSuccess(''); setIsLoading(true);
    try {
      await api.post('/auth/reset-password', { email: recoveryEmail, code: resetCode, newPassword });
  ...
  const switchMode = (newMode: ModalMode) => {
    ...
    setShowPassword(false); setShowNewPassword(false); setResetCode('');
  };
  ...
  const getSubtitle = () => {
    switch (mode) {
      case 'login': return 'Accede a tu cuenta de Sushi de Maksim';
      case 'register': return 'Únete a Sushi de Maksim';
      case 'forgot': return 'Introduce tu email para recuperar el acceso';
      case 'reset': return `Establece una nueva contraseña para ${recoveryEmail}`;
    }
  };
      ...
      <div className="bg-white rounded-[20px] p-7 w-[90%] max-w-[380px] max-h-[90vh] overflow-y-auto relative shadow-2xl animate-[slideUp_0.3s_ease]">
        ...
        <div className="text-center mb-5">
          <div className="w-[120px] h-[120px] rounded-2xl flex items-center justify-center mx-auto -mt-4 mb-3 bg-white">
            {isForgotOrReset ? (
              ...
              <img src="/logo.svg" alt="Sushi de Maksim" className="w-[120px] h-[120px] object-contain" />
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="mb-4">
              ...
              <>
                <div className="mb-3">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Tu nombre completo" required className="modal-input" />
                  </div>
                </div>
                <div className="mb-3">
                  <label className="block mb-1 text-xs font-semibold text-gray-700">Teléfono</label>
                  <div className="relative">
                    <Phone size={15} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+34 600 000 000" required className="modal-input" />
                  </div>
                </div>
              </>
            )}
            ...
            <div className="mb-3">
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="tu@email.com" required className="modal-input" />
              </div>
            </div>

            <div className={mode === 'login' ? 'mb-2' : 'mb-4'}>
              <label className="block mb-1 text-xs font-semibold text-gray-700">Contraseña</label>
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 border-none bg-transparent cursor-pointer p-0.5 text-gray-400">
            ...
            {mode === 'login' && (
              <div className="text-right mb-3">
                <button type="button" onClick={() => switchMode('forgot')} className="border-none bg-transparent text-red-600 text-xs cursor-pointer p-0 font-medium hover:underline">
            ...
            >
              {isLoading ? '...' : mode === 'login' ? 'Iniciar sesión' : 'Crear cuenta'}
            </button>
          </form>
        )}
        ...
        {mode === 'forgot' && (
          <form onSubmit={handleForgotSubmit} className="mb-4">
            <div className="mb-4">
              <label className="block mb-1 text-xs font-semibold text-gray-700">Email de tu cuenta</label>
              <div className="relative">
                <Mail size={15} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="tu@email.com" required autoComplete="email" autoFocus className="modal-input" />
              </div>
            </div>
        ...
        {mode === 'reset' && (
          <form onSubmit={handleResetSubmit} className="mb-4">
            <div className="mb-3">
              <label className="block mb-1 text-xs font-semibold text-gray-700">Código de 6 dígitos</label>
              <input
                type="text"
                value={resetCode}
            ...
            <div className="mb-3">
              <label className="block mb-1 text-xs font-semibold text-gray-700">Nueva contraseña</label>
              <div className="relative">
                <Lock size={15} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input
                  ...
                  className="modal-input-pwd"
                />
                <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 border-none bg-transparent cursor-pointer p-0.5 text-gray-400">
            ...
            <div className="mb-4">
              <label className="block mb-1 text-xs font-semibold text-gray-700">Confirmar contraseña</label>
              <div className="relative">
                <Lock size={15} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input
                  type={showNewPassword ? 'text' : 'password'}
        ...
        {(mode === 'login' || mode === 'register') && (
          <div className="text-center">
            ...
            <button onClick={() => switchMode(mode === 'login' ? 'register' : 'login')} className="border-none bg-transparent text-red-600 font-bold cursor-pointer ml-1 text-xs">
        ...
        {isForgotOrReset && (
          <div className="text-center">
            ...
            <button onClick={() => switchMode('login')} className="border-none bg-transparent text-red-600 font-bold cursor-pointer ml-1 text-xs">
              Inicia sesión
            </button>
          </div>
        )}
