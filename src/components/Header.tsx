...
import { ShoppingCart, User, ChevronDown, LogOut, Menu, X, ShieldCheck, ChevronRight } from 'lucide-react';
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex items-center no-underline gap-2 group">
              <div className="bg-red-600 p-1.5 rounded-lg group-hover:rotate-12 transition-transform duration-300">
                <img src="/logo.svg" alt="Sushi de Maksim" className="h-6 w-auto brightness-0 invert" />
              </div>
            ...
            <div className="flex items-center gap-3">
                      ...
                      {showUserMenu && (
                        ...
                        >
                          <div className="px-4 py-3 border-b border-gray-50 mb-1">
                            <p className="text-sm font-black text-gray-900 mb-0.5">{user.name}</p>
              ...
              {/* Cart */}
              <Link id="cart-icon" to="/cart" className="relative p-2.5 no-underline text-gray-800 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors">
            ...
            >
              <div className="p-4 space-y-2">
                {navLinks.map(link => (
                  ...
                  >
                    {link.label}
                    {location.pathname === link.to && <ChevronRight size={16} className="ml-auto" />}
                  </Link>
                ))}
                ...
                {isAuthenticated && user ? (
                  <div className="space-y-2">
                    <div className="px-4 py-2">
                      <p className="font-black text-gray-900 text-sm mb-0.5">{user.name}</p>
                      <p className="text-xs text-gray-400 font-medium">{user.email}</p>
                    </div>
                    {user.role === 'admin' && (
                      <Link to="/admin" className="flex items-center gap-2.5 px-4 py-3.5 rounded-2xl no-underline text-red-600 text-sm font-black bg-red-50">
                        <ShieldCheck size={18} /> PANEL ADMIN
                      </Link>
                    )}
                    <Link to="/profile" className="flex items-center gap-2.5 px-4 py-3.5 rounded-2xl no-underline text-gray-700 text-sm font-bold">
