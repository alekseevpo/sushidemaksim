...
1.  **Search Logic Flaw**: In `AdminOrders.tsx` and `AdminUsers.tsx`, searching is performed on the frontend using `filter()`. Since the API uses pagination, this only searches the *current page* of results. Search queries should be handled by the backend.
