    ...
    for (const item of menuItems) {
      if (item.image && !item.image.endsWith('.webp')) {
        try {
          ...
          const optimized = await processImage(Buffer.from(response.data), { type: 'menu' });
          
          const newFileName = `${Date.now()}-${item.id}.webp`;
          const filePath = `menu/${newFileName}`;
          
          const { error: uploadError } = await supabase.storage.from('images').upload(filePath, optimized, {
          ...
          if (!uploadError) {
            ...
            await supabase.from('menu_items').update({ image: publicUrl }).eq('id', item.id);
    ...
    for (const user of users) {
      if (user.avatar && !user.avatar.endsWith('.webp') && user.avatar.includes('supabase')) {
        try {
          ...
          const optimized = await processImage(Buffer.from(response.data), { type: 'avatar' });
      ...
      if (post.image_url && !post.image_url.endsWith('.webp')) {
        try {
          ...
          if (post.image_url.startsWith('http')) {
            ...
            const rootPublicPath = path.join(process.cwd(), '..', 'public', post.image_url);
            ...
            try {
              ...
              try {
                ...
                throw new Error(`File not found in root public or server public: ${post.image_url}`);
              }
            }
          ...
          if (!uploadError) {
            const { data: { publicUrl } } = supabase.storage.from('images').getPublicUrl(filePath);
            await supabase.from('blog_posts').update({ image_url: publicUrl }).eq('id', post.id);
